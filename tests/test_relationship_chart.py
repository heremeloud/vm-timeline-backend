...
class RelationshipChartTests(unittest.TestCase):
    def test_chart_text_and_groups_round_trip_and_validate_members(self):
        ...
        restored = RelationshipChartData.model_validate_json(RelationshipChartData.model_validate(data).model_dump_json())
        ...
        with self.assertRaises(ValidationError):
            RelationshipChartData.model_validate(data)
    ...
    def test_thai_name_and_database_or_manual_actor(self):
        ...
        with Session(engine) as session:
            ...
            saved = update_project(project.id, ProjectUpdate(relationship_chart=data), session)
            self.assertEqual(saved["relationship_chart"]["characters"][0]["thai_name"], "ขนมปัง")
            self.assertEqual(saved["relationship_chart"]["characters"][0]["actor"], author.name)
            author.name = "Updated actor name"
            session.add(author)
            session.commit()
            reloaded = _serialize_project(session, project)
            self.assertEqual(reloaded["relationship_chart"]["characters"][0]["actor"], author.name)
            data["characters"][0].update(author_id=None, actor="Manual actor")
            saved = update_project(project.id, ProjectUpdate(relationship_chart=data), session)
            self.assertIsNone(saved["relationship_chart"]["characters"][0]["author_id"])
            self.assertEqual(saved["relationship_chart"]["characters"][0]["actor"], "Manual actor")
        engine.dispose()
    ...
    def test_hide_reload_and_restore_preserves_chart(self):
        ...
        with Session(engine) as session:
            ...
            saved = update_project(project_id, ProjectUpdate(relationship_chart=chart()), session)
            update_project(project_id, ProjectUpdate(show_relationship_chart=False), session)
        with Session(engine) as session:
            reloaded = _serialize_project(session, session.get(Project, project_id))
            self.assertFalse(reloaded["show_relationship_chart"])
            self.assertEqual(reloaded["relationship_chart"], saved["relationship_chart"])
            restored = update_project(project_id, ProjectUpdate(show_relationship_chart=True), session)
            self.assertTrue(restored["show_relationship_chart"])
            self.assertEqual(restored["relationship_chart"], saved["relationship_chart"])
            emptied = update_project(project_id, ProjectUpdate(relationship_chart={"characters": [], "relationships": []}), session)
            self.assertEqual(emptied["relationship_chart"]["characters"], [])
        engine.dispose()
